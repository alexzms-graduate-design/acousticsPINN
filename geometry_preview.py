...
def main():
    ...
    if 1 in grids:
        p.add_mesh(grids[1], show_edges=True, color="lightgray", opacity=0.8, label="Pipe Wall")
    else:
        print("物理组 1（管壁）未找到")
    if 2 in grids:
        p.add_mesh(grids[2], show_edges=True, color="skyblue", opacity=0.7, label="Fluid Domain")
    else:
