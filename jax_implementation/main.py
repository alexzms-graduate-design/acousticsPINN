...
# 定义入口激励：在流体域（dof_per_node==1）的入口节点 (x≈0) 固定 p=10.0 Pa
...
class JaxFEMMicPressure(torch.autograd.Function):
    ...
    def forward(ctx, E_norm_t, nu_t, rho_norm_t):
        # 将 torch 参数转换为 numpy/jax 数组（标量），保持梯度信息
        E_norm_np = E_norm_t.cpu().numpy()
        nu_np = nu_t.cpu().numpy()
        rho_norm_np = rho_norm_t.cpu().numpy()
        ...
        # 计算梯度
        grad_fn = jax.grad(fem_obj, argnums=(0,1,2))
        grads = grad_fn(float(E_norm_np), float(nu_np), float(rho_norm_np))
        grad_E_norm, grad_nu, grad_rho_norm = grads
        print(f"------------grad_E_norm: {grad_E_norm}, grad_nu: {grad_nu}, grad_rho_norm: {grad_rho_norm}---------------")
    ...
    def backward(ctx, grad_output):
        ...
        # 确保梯度形状匹配
        dE_norm = grad_output * grad_E_norm
        dnu = grad_output * grad_nu
        drho_norm = grad_output * grad_rho_norm
